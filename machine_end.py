...
def write_machine_data_target(game_name):
    ...
    for j in range(0, len(data_train_temp), 9):
        ...
        data_train.append(float(data_train_temp[j + 7][3].replace('%', '')) / 100)
    # target
    for i in range(0, len(data_train_temp), 9):
        date_target.append(float(data_train_temp[i+8][3].replace('%', '')) / 100)
    ...
    with open(f'datasets/{game_name}/Machine_{game_name}_end.json', 'w', encoding='utf-8') as f:
        f.write(data)
...
def machine_learn_ada_decisiontree():
    ...
    x_test, y_test = x_train_temp[num_training:], y_train_temp[num_training:]
    pprint(num_training)
    dt_regressor = DecisionTreeRegressor(max_depth=8, criterion='mse', min_samples_leaf=2, splitter='best')
    dt_regressor.fit(x_train, y_train)

    ad_regressor = AdaBoostRegressor(
        DecisionTreeRegressor(max_depth=8, criterion='mse', min_samples_leaf=2, splitter='best'), n_estimators=800,
        random_state=7)
...
def machine_learning_mul_randomforest():
    ...
    # 定义模型
    regr_rf = MultiOutputRegressor(RandomForestRegressor(n_estimators=50, max_depth=10,
                                                         random_state=2))
    # 集合模型
    regr_rf.fit(x_train, y_train)
    # 利用预测
...
def save_model():
    ...
    output_model_file = 'saved_model_end.pkl'
    with open(output_model_file, 'wb') as f:
        pickle.dump(model, f)


def use_model():
    with open('saved_model_end.pkl', 'rb') as f:
...
# machine_learn_ada_decisiontree()
